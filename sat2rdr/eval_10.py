...
def val():
    ...
    for idx, batchs in pbar:
        ...
        with torch.no_grad():
            ...
            #pred_low = transforms.resize(pred, (pred.shape[0],pred.shape[1],pred.shape[2] // 5, pred.shape[3] // 5), preserve_range = True)
            #targets_low = transforms.resize(targets, (targets.shape[0], targets.shape[1], targets.shape[2] //5 ,targets.shape[3] //5), preserve_range = True)
            
            pred = F.interpolate(pred, size=(45, 45), mode = 'bilinear', align_corners=False)
            targets = F.interpolate(targets, size = (45, 45), mode = 'bilinear', align_corners=False)
