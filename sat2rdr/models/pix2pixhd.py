    ...
    def __init__(self, 
        ...
        for _ in range(n_residual):
            if mid_type == 'resnet':
                ...
                model += [VANSubBlock(n_gf, mlp_ratio=8, drop=0, drop_path=0, act_layer=nn.GELU)]
            elif mid_type == 'convnext':
                model += [ConvNeXtSubBlock(n_gf,  mlp_ratio=8, drop=0, drop_path=0)]
            elif mid_type == 'vit':
                model += [ViTSubBlock(n_gf, mlp_ratio=8, drop=0, drop_path=0)]
            else:
                print('model error')
